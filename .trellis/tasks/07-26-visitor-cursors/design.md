...
- `mousemove` 或等价鼠标指针事件只更新内存坐标，不直接发起网络请求。
...
- 展示层属于全局非交互装饰，使用语义化 z-index token，低于 modal/toast/tooltip。
